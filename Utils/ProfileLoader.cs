...
namespace DiabloTwoMFTimer.Utils
{
    public static class ProfileLoader
    {
        ...
        private static readonly Dictionary<string, CharacterProfile> _profileCache =
        ...
        private static CharacterProfile? LoadProfileFromFile(string filePath)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (profile == null)
                    {
                        LogManager.WriteDebugLog(
                            "ProfileLoader",
                            $"解析文件 {Path.GetFileName(filePath)} 返回null，创建默认角色"
                        );
                ...
                catch (Exception ex)
                {
                    LogManager.WriteDebugLog(
                        "ProfileLoader",
                        $"处理文件 {Path.GetFileName(filePath)} 时出错: {ex.Message}"
                    );
                    profile = new CharacterProfile() { Name = Path.GetFileNameWithoutExtension(filePath) };
                }
